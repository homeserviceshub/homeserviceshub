...
const RequestaService = () => {
  ...
  const clientID = useLocation().pathname.split("/")[1];
  const timings = [
  ...
  const fetchData = async () => {
    try {
      ...
      if (userResponse.status === 200) {
        ...
        setClientData(userResponse.data[0].aceData);
      }
    } catch (error) {
        ...
        <div key={1} ref={secondCardRef} id="secondCard" data-aos="fade-up">
          <div className={styles.main}>
            ...
            <div className={styles.bothIcons}>
              ...
              <div
                className={`${styles.icon} ${styles.iconBuilding} ${
